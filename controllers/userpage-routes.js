...
//get all of the current users posts (.../userpage)
router.get('/', auth,(req, res) => {
    Post.findAll({
        ...
        .then(dbPostData => {
            ...
            const posts = dbPostData.map(post => post.get({ plain: true }));
            res.render('myuserpage',{ posts, loggedIn: req.session.loggedIn });
        })
        .catch(err => {
            console.log(err);
            res.status(500).json(err);
...
router.get('/edit/:id', (req, res) => {
    Post.findOne({
        ...
        .then(dbPostData => {
            ...
            res.render('editpage',{ post, loggedIn: req.session.loggedIn });
        })
        .catch(err => {
            console.log(err);
            res.status(500).json(err);
